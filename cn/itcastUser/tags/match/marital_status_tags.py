...
if __name__ == '__main__':
    ...
    tag_df.show()
    rule_df = tag_df.where('id=65').select('rule')
    ...
    tag_five_df: DataFrame = tag_df.where('pid=65').select('rule', 'id')
    tag_five_df.show()
    ...
    def get_tags(marriage):
        return broadcast_dict.value.get(str(marriage))

    res_df: DataFrame = es_df.select(es_df['id'].alias('userId'), get_tags(es_df['marriage']).alias('tagsId'))
    ...
    new_tags_df.write \
        ...
        .option('es.resource', 'marital_status_userprofile_result/_doc')\
